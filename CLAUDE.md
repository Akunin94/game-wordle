...
| 8 слов убраны из `VALID_GUESSES` (qumri, rahat, rahim, qator, sifat, havas, karam, mezon+katta) | Были в обоих массивах — тест `words.test.ts` проверяет `∩ = ∅` |
| jest конфиг встроен прямо в `package.json` (нет отдельного `jest.config.js`) | Минимум файлов; `moduleNameMapper` закрывает алиас `@/` |
